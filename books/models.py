...
class AddBooks(models.Model):
    ...
    image = models.ImageField(
        upload_to="images/", verbose_name="Загрузите фото", null=True
    )
    description = models.TextField(verbose_name="Напишите описание книги")
    audio_book = models.FileField(
    ...
    book_genre = models.CharField(
        max_length=100, choices=BOOK_GENRE, verbose_name="Выберите жанр"
    )
...
class Review(models.Model):
    review_book = models.ForeignKey(
        AddBooks, on_delete=models.CASCADE, related_name="review_book", null=True
    )
